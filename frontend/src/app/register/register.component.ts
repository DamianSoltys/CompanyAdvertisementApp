      ...
      (data: HttpResponse<any>) => {
        if (data.status === 200) {
          ...
          this.showRequestMessage(
            'success',
            'Proszę potwierdzić swoje konto linkiem wysłanym na podany adres email',
            ''
          );
          console.log(
            'Proszę potwierdzić swoje konto linkiem wysłanym na podany email!'
          );
        }
      },
      error => {
        this.showRequestMessage(
          'error',
          '',
          'Niestety rejestracja się nie udała!'
        );
